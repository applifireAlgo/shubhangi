 ...
 * ! 
 * 
 * Copyright 2011, Dan Harabagiu Licenced under the Apache License Version 2.0
 * See LICENSE
 * 
 * 
 * Version : 0.1 - Initial coding Version : 0.2 - Added Field reset button -
 * Added CSS class for reset button - Added reset function for the field -
 * Minimum number of stars is 2 - On creation default value is now 0, was null -
 * Option to choose left / right for the reset button position
 */
/* global Ext : false, */

Ext.define(
				'Testapp3.view.searchengine.search.PanelHeaderField',
				{
					extend : 'Ext.form.Field',
					alias : 'widget.panelheaderfield',
//					requires : [ 'Ext.form.field.VTypes','Ext.layout.component.field.Text' ],
					requires : ['Ext.form.field.VTypes', 'Ext.form.field.Text'],
					...
					onRender : function(ct, position) {
						Testapp3.view.searchengine.search.PanelHeaderField.superclass.onRender.call(this, ct,
								position);
						...
						for (var i = 1; i <= this.numberOfStars; i++) {
							...
							starElement.setAttributeNode(this
									.createHtmlAttribute("key", i));
						...
						inputElement.setAttributeNode(this.createHtmlAttribute(
								"type", "hidden"));
						inputElement.setAttributeNode(this.createHtmlAttribute(
								"name", this.getName()));
						...
						limitStar = this.value;
						this.reset();

						if (limitStar != undefined) {
...
//						Ext.ux.RatingField.superclass.initEvents.call(this);
						Testapp3.view.searchengine.search.PanelHeaderField.superclass.initEvents.call(this);

						for (var i = 0; i < this.stars.length; i++) {
							this.stars[i]
									.on('mouseenter', this.showStars, this);
							this.stars[i]
									.on('mouseleave', this.hideStars, this);
					...
					reset : function() {
						for (var i = 0; i < this.stars.length; i++) {
							if (this.stars[i].hasCls(this.ratingClassOn) === true
									&& this.stars[i]
											.hasCls(this.ratingClassSelected) === true) {
								this.stars[i].replaceCls(this.ratingClassOn,
										this.ratingClassOff);
								this.stars[i]
										.removeCls(this.ratingClassSelected);
							}
					...
					selectStars : function(e, t) {
						...
						for (i = 0; i < limitStar; i++) {
							if (this.stars[i].hasCls(this.ratingClassOn) === false) {
								this.stars[i].replaceCls(this.ratingClassOff,
										this.ratingClassOn);
							}
							this.stars[i].addCls(this.ratingClassSelected);
						}
					...
					/**
					 * Based on hover, show the amount of stars that will be
					 * selected
					 * 
					 * @param {Ext.EventObject}
					 *            e
					...
					showStars : function(e, t) {
						...
						for (var i = 0; i < limitStar; i++) {
							if (this.stars[i].hasCls(this.ratingClassOn) === false
									&& this.stars[i]
											.hasCls(this.ratingClassSelected) === false) {
								this.stars[i].replaceCls(this.ratingClassOff,
										this.ratingClassOn);
							}
						}
					...
					hideStars : function() {
						for (var i = 0; i < this.stars.length; i++) {
							if (this.stars[i].hasCls(this.ratingClassOff) === false
									&& this.stars[i]
											.hasCls(this.ratingClassSelected) === false) {
								this.stars[i].replaceCls(this.ratingClassOn,
										this.ratingClassOff);
					...
					/**
					 * Private function, that ads a html attribute to a dom
					 * element
					 * 
					 * @param {string}
