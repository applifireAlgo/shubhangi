Ext.define('Testapp3.view.usermanagement.admin.ViewUserController', {
	extend : 'Ext.app.ViewController',
	alias : 'controller.viewUserController',
	requires:['Testapp3.view.usermanagement.admin.UserForm'],
        ...
        Ext.Ajax.request(
    	{
    		...
    		success : function(response, currentObject,options) 
    		{
    			...
    			if(jsonResponse.response.success==true)
            	{
					...
					{
						var displayName = data[i].coreContacts.firstName + " ";
	    				if(data[i].coreContacts.middleName && data[i].coreContacts.middleName!=null) {
	    					displayName = displayName + data[i].coreContacts.middleName + " ";
	    				}
	    				displayName = displayName + data[i].coreContacts.lastName;
    	...
    	Ext.Ajax.request({
    	    ...
    	    success : function(response, currentObject,options) 
    	    {
    	    	...
    	    	if(jsonResponse.response.success==true)
            	{
	    			...
	    			currentObject.me.loadTimezoneCombo(form.down('#timezoneCombo'),data[0].coreContacts.timezone.timeZoneId);
	    			...
	    			if(data[0].coreContacts.dateofbirth!=null && data[0].coreContacts.dateofbirth!=""){ 
	    				var dateWithoutTimezone = data[0].coreContacts.dateofbirth.replace(' ' + Ext.util.Cookies.get('XA_TID'), '');
	...
	loadTimezoneCombo : function(combo, value) {
		Ext.Ajax.request({
