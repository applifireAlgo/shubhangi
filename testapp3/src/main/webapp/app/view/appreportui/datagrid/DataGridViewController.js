Ext.define('Testapp3.view.appreportui.datagrid.DataGridViewController', {
	...
	openDrillDown : function(reportId, reportName, rptHrefQC,gridObj,targetObj) 
	{
		...
		if(targetObj.targetSrc=="tab"){
			...
			if(targetObj.type == "f"){
				...
				component= Ext.create(
						'Testapp3.view.appreportui.ReportView', {
							closable : true,
							drilled : true,		
							title : reportName,
							refId : reportId,
	...
	onTargetReportSelect:function(combo, records, eOpts ){
		Ext.Ajax.request({
			url : 'secure/dataEngineWebService/getReportDetailsById?reportId=' + records.data.report_id,
			method : 'POST',
			async:false,
			window:this.up().up(),
			jsonData : {},
			success : function(response,currentObject, options) {
				...
				if (responseJson.response.success == true) {
					...
					currentObject.window.menuObj.scope.filterColumnList(data.data_json.queryCWidgetU,currentObject.window);
				} else {
