Ext.define('Testapp3.view.clouddrive.documentview.DocumentListViewController',{
	extend : 'Ext.app.ViewController',
	requires:['Testapp3.view.fw.PDF.panel.PDF'],
	...
	downloadFiles : function(fileId) 
	{
		...
		Ext.Ajax.request({
			url : "secure/cloudDriveController/downloadFilePost?fileId="+ fileId,
			method : 'POST',
			entMask:entMask,
			scope:this,
			params : {
			...
			success : function(response,currentObject) {
				...
				currentObject.scope.downloadFileform.dom.action = "secure/cloudDriveController/downloadFilePost?fileId="
					+ fileId, currentObject.scope.downloadFileform.dom.submit();
			},
	...
	getManageRevGridStore:function()
	{
		return new Ext.create('Ext.data.Store', {
			fields : ['baseFileId','fileId','updatedByFname','createdDate','fileSize'],  
			data : []
		});
	},
