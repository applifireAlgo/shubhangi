 ...
 * 
 */
Ext.define("Testapp3.view.scheduleconfiguration.panel.ScheduleConfigController", {
...
//					var data = Ext.JSON.decode(jsonResponse.response.data);
					currentObject.combo.getStore().loadData(jsonResponse.response.data);
				} else {
					Ext.Msg.alert({
						title : 'Error...',
	...
	prepareJSON : function(formData) {
		var jsonData = {};
		jsonData.schedulerId = this.getView().scheduleId;
		jsonData.jobDetails = {
		...
		Ext.Ajax.request({
			...
			success : function(response, opts) {
				...
				if (responseJson.response.success == true) {
					...
					// Load data to fields
					currentObject.getView().scheduleId = data.schedulerId;
