 ...
 * 
 */
Ext.define('Testapp3.view.securityconfig.SecurityConfigController',{
...
//		currentObject.selectedPolicyId = currentObject.view.down('#selectedPolicyId');
	...
	loadFormData : function()
	{
		...
		Ext.Ajax.request({
			timeout: 6000000,
			url : 'secure/ArtPasswordAlgorithm/findAll',
			method : 'GET',
			jsonData : {},
					...
					/*Ext.Ajax.request({
						...
						success : function(response, opts){
							...
							if (responseJson.response.success) {
								...
								Ext.Ajax.request({
									url : 'secure/PasswordAlgo/findAll',
									method : 'GET',
									jsonData : {},
									success : function(response, opts){
										...
										if (responseJson.response.success) {
											...
											currentObject.algoVersionId = data.versionId;
											...
											currentObject.encryption.setValue(data.algorithm);
										} else {
		...
		Ext.Ajax.request({
			timeout: 6000000,
			url : 'secure/PasswordPolicy/findAll',
			method : 'GET',
			jsonData : {},
			success : function(response, opts){
	...
	onStrengthSelection : function(combo, records, eOpts)
	{
		...
		this.renderDefaultPasswordPolicy(records.get('strength'), records.get('strengthId'));
	},
	...
	onSubmitClick : function(but, evt) {
		...
		if (form.isValid()) {
			Ext.Msg.confirm('WARNING',
					'Changing security setting will affect the default users!<br>Recommendation - If you are changing password policy or algorithm change all default users password according to new settings.<br>Are you sure you want to proceed?',
					function(id, value) {
				if (id == 'yes') {
