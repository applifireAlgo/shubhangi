Ext.define('Testapp3.testapp3.web.com.controller.salesboundedcontext.sales.CurrentMonthMainController', {
     extend: 'Testapp3.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CurrentMonthMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CurrentMonthTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#CurrentMonthForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#CurrentMonthGrid');
               var tree = this.view.down('#CurrentMonthTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#CurrentMonthForm');
          var grid = this.view.down('#CurrentMonthGrid');
          var tree = this.view.down('#CurrentMonthTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#CurrentMonthGrid');
               ...
               var formPanel = this.getView().up().down('#CurrentMonthForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#CurrentMonthTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CurrentMonthForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CurrentMonthGrid').store;
          gridStore.removeAll();
