Ext.define('Testapp3.testapp3.web.com.controller.salesboundedcontext.sales.DistributorMainController', {
     extend: 'Testapp3.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.DistributorMainController',
     ...
     regioncodeLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/SalesRegion/findAll',
               method: 'GET',
               async: false,
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#regioncode');
                    scope.sender.controller.assignAllComboData(scope.sender, 'regioncode', response.responseText);
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#DistributorTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#DistributorForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#DistributorGrid');
               var tree = this.view.down('#DistributorTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#DistributorForm');
          var grid = this.view.down('#DistributorGrid');
          var tree = this.view.down('#DistributorTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#DistributorGrid');
               ...
               var formPanel = this.getView().up().down('#DistributorForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#DistributorTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#DistributorForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#DistributorGrid').store;
          gridStore.removeAll();
