Ext.define('Testapp3.testapp3.web.com.controller.organization.contactmanagement.CommunicationGroupMainController', {
     extend: 'Testapp3.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CommunicationGroupMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CommunicationGroupTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#CommunicationGroupForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#CommunicationGroupGrid');
               var tree = this.view.down('#CommunicationGroupTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#CommunicationGroupForm');
          var grid = this.view.down('#CommunicationGroupGrid');
          var tree = this.view.down('#CommunicationGroupTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#CommunicationGroupGrid');
               ...
               var formPanel = this.getView().up().down('#CommunicationGroupForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#CommunicationGroupTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CommunicationGroupForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CommunicationGroupGrid').store;
          gridStore.removeAll();
