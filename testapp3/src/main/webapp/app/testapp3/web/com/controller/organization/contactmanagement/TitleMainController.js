Ext.define('Testapp3.testapp3.web.com.controller.organization.contactmanagement.TitleMainController', {
     extend: 'Testapp3.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.TitleMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#TitleTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#TitleForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#TitleGrid');
               var tree = this.view.down('#TitleTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#TitleForm');
          var grid = this.view.down('#TitleGrid');
          var tree = this.view.down('#TitleTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#TitleGrid');
               ...
               var formPanel = this.getView().up().down('#TitleForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#TitleTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#TitleForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#TitleGrid').store;
          gridStore.removeAll();
