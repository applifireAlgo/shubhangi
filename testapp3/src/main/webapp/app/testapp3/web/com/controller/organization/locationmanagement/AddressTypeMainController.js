Ext.define('Testapp3.testapp3.web.com.controller.organization.locationmanagement.AddressTypeMainController', {
     extend: 'Testapp3.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.AddressTypeMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#AddressTypeTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#AddressTypeForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#AddressTypeGrid');
               var tree = this.view.down('#AddressTypeTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#AddressTypeForm');
          var grid = this.view.down('#AddressTypeGrid');
          var tree = this.view.down('#AddressTypeTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#AddressTypeGrid');
               ...
               var formPanel = this.getView().up().down('#AddressTypeForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#AddressTypeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#AddressTypeForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#AddressTypeGrid').store;
          gridStore.removeAll();
