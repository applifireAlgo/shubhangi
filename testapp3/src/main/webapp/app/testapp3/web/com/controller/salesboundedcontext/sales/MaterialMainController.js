Ext.define('Testapp3.testapp3.web.com.controller.salesboundedcontext.sales.MaterialMainController', {
     extend: 'Testapp3.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.MaterialMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#MaterialTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#MaterialForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#MaterialGrid');
               var tree = this.view.down('#MaterialTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#MaterialForm');
          var grid = this.view.down('#MaterialGrid');
          var tree = this.view.down('#MaterialTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#MaterialGrid');
               ...
               var formPanel = this.getView().up().down('#MaterialForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#MaterialTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#MaterialForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#MaterialGrid').store;
          gridStore.removeAll();
