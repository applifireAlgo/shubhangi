Ext.define('Testapp3.testapp3.web.com.controller.salesboundedcontext.sales.CategoryMainController', {
     extend: 'Testapp3.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CategoryMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CategoryTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#CategoryForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#CategoryGrid');
               var tree = this.view.down('#CategoryTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#CategoryForm');
          var grid = this.view.down('#CategoryGrid');
          var tree = this.view.down('#CategoryTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#CategoryGrid');
               ...
               var formPanel = this.getView().up().down('#CategoryForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#CategoryTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CategoryForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CategoryGrid').store;
          gridStore.removeAll();
