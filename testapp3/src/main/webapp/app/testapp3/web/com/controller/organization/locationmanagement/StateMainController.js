Ext.define('Testapp3.testapp3.web.com.controller.organization.locationmanagement.StateMainController', {
     extend: 'Testapp3.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.StateMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#StateTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#StateForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#StateGrid');
               var tree = this.view.down('#StateTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#StateForm');
          var grid = this.view.down('#StateGrid');
          var tree = this.view.down('#StateTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#StateGrid');
               ...
               var formPanel = this.getView().up().down('#StateForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#StateTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#StateForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#StateGrid').store;
          gridStore.removeAll();
