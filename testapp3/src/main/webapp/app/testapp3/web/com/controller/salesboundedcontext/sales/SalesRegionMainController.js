Ext.define('Testapp3.testapp3.web.com.controller.salesboundedcontext.sales.SalesRegionMainController', {
     extend: 'Testapp3.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.SalesRegionMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#SalesRegionTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#SalesRegionForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#SalesRegionGrid');
               var tree = this.view.down('#SalesRegionTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#SalesRegionForm');
          var grid = this.view.down('#SalesRegionGrid');
          var tree = this.view.down('#SalesRegionTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#SalesRegionGrid');
               ...
               var formPanel = this.getView().up().down('#SalesRegionForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#SalesRegionTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#SalesRegionForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#SalesRegionGrid').store;
          gridStore.removeAll();
