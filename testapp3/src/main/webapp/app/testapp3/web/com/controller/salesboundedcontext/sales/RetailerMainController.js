Ext.define('Testapp3.testapp3.web.com.controller.salesboundedcontext.sales.RetailerMainController', {
     extend: 'Testapp3.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.RetailerMainController',
     /*********************Main Controller Functions*********************************/
     init: function() {
          var but = this.view.down('#resetFormButton');
          but.up('form').reset();
          this.distributorcodeLoad();
     ...
     distributorcodeLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/Distributor/findAll',
               method: 'GET',
               async: false,
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#distributorcode');
                    scope.sender.controller.assignAllComboData(scope.sender, 'distributorcode', response.responseText);
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#RetailerTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#RetailerForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#RetailerGrid');
               var tree = this.view.down('#RetailerTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#RetailerForm');
          var grid = this.view.down('#RetailerGrid');
          var tree = this.view.down('#RetailerTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#RetailerGrid');
               ...
               var formPanel = this.getView().up().down('#RetailerForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#RetailerTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#RetailerForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#RetailerGrid').store;
          gridStore.removeAll();
