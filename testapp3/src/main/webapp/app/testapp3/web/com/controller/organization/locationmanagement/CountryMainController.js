Ext.define('Testapp3.testapp3.web.com.controller.organization.locationmanagement.CountryMainController', {
     extend: 'Testapp3.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CountryMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CountryTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#CountryForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#CountryGrid');
               var tree = this.view.down('#CountryTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#CountryForm');
          var grid = this.view.down('#CountryGrid');
          var tree = this.view.down('#CountryTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#CountryGrid');
               ...
               var formPanel = this.getView().up().down('#CountryForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#CountryTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CountryForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CountryGrid').store;
          gridStore.removeAll();
