Ext.define('Testapp3.testapp3.web.com.controller.organization.contactmanagement.ContactTypeMainController', {
     extend: 'Testapp3.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.ContactTypeMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#ContactTypeTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#ContactTypeForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#ContactTypeGrid');
               var tree = this.view.down('#ContactTypeTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#ContactTypeForm');
          var grid = this.view.down('#ContactTypeGrid');
          var tree = this.view.down('#ContactTypeTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#ContactTypeGrid');
               ...
               var formPanel = this.getView().up().down('#ContactTypeForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#ContactTypeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#ContactTypeForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#ContactTypeGrid').store;
          gridStore.removeAll();
