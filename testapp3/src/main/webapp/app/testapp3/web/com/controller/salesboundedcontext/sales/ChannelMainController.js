Ext.define('Testapp3.testapp3.web.com.controller.salesboundedcontext.sales.ChannelMainController', {
     extend: 'Testapp3.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.ChannelMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#ChannelTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#ChannelForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#ChannelGrid');
               var tree = this.view.down('#ChannelTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#ChannelForm');
          var grid = this.view.down('#ChannelGrid');
          var tree = this.view.down('#ChannelTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#ChannelGrid');
               ...
               var formPanel = this.getView().up().down('#ChannelForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#ChannelTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#ChannelForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#ChannelGrid').store;
          gridStore.removeAll();
